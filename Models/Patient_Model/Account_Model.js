...
const createPatient = async (email, password , name) => {
  try {
    const checkPatient = await pool.query("SELECT * FROM patients WHERE email = $1", [email]);
...
const checkEmail = async (email) => {
  try {
    ...
    if (response.rows.length > 0) {
      ...
      const result = await pool.query("UPDATE patients SET reset_code = $1, code_expires_at = $2 WHERE email = $3", [verificationNumber, expiresAt, email]);
...
const resetPassword = async (email, code, password) => {
  try {
    ...
    const userCheck = await pool.query("SELECT * FROM patients WHERE email = $1", [email]);
